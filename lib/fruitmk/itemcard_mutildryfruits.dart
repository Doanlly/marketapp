...
class ItemcardmutilDryF extends StatefulWidget {
  var press;

  ItemcardmutilDryF({Key? key, required this.mixededryf, required this.press})
      : super(key: key);
  final MixeddeyFruits mixededryf;

  //get press => null;
  //Function press;

  @override
  State<ItemcardmutilDryF> createState() => _ItemcardmutilDryFState();
}

class _ItemcardmutilDryFState extends State<ItemcardmutilDryF> {
        ...
        children: [
                    ...
                    child: InkWell(
                        onTap: () {
                          ...
                          if (checkfavorrite == false) {
                            setidf.add("${widget.mixededryf.id}");
                            //print("${Listitemproduc}");
                            print("setid: ${setidf}");
                            for (int i = 0; i < setidf.length; i++) {
                              for (int j = 0; j < mixerdryfruit.length; j++) {
                                if (setidf[i] == "${mixerdryfruit[j].id}") {
                                  savedataproductf.add(mixerdryfruit[j].title);
                                  savedataproductf.add(mixerdryfruit[j].price);
                                  savedataproductf.add(mixerdryfruit[j].image);
                                  savedataproductf
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Text(
                "${widget.mixededryf.price} Per/ kg",
