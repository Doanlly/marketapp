...
class MelonsItem extends StatefulWidget {
  MelonsItem({Key? key, required this.melonz, required this.press})
      : super(key: key);
  Melons melonz;
  var press;

  @override
  State<MelonsItem> createState() => _MelonsItemState();
}

class _MelonsItemState extends State<MelonsItem> {
                    ...
                    child: InkWell(
                        onTap: () {
                          ...
                          if (checkfavorrite1 == false) {
                            setidf.add("${widget.melonz.id}");
                            //print("${Listitemproduc}");
                            print("setid: ${setidf}");
                            for (int i = 0; i < setidf.length; i++) {
                              for (int j = 0; j < melonss.length; j++) {
                                if (setidf[i] == "${melonss[j].id}") {
                                  savedataproductf.add(melonss[j].title);
                                  savedataproductf.add(melonss[j].price);
                                  savedataproductf.add(melonss[j].image);
                                  savedataproductf
                                      .add(melonss[j].phanloai);
                                }
                              }
                            }
