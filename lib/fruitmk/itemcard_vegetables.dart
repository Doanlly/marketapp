...
class ItemcardVegetables extends StatefulWidget {
  var press;

  ItemcardVegetables({Key? key, required this.organic, required this.press})
      : super(key: key);
  final Organicvegetables organic;

  //get press => null;
  //Function press;

  @override
  State<ItemcardVegetables> createState() => _ItemcardVegetablesState();
}

class _ItemcardVegetablesState extends State<ItemcardVegetables> {
                    ...
                    child: InkWell(
                        onTap: () {
                          ...
                          if (checkfavorrite == false) {
                            setidf.add("${widget.organic.id}");
                            //print("${Listitemproduc}");
                            print("setid: ${setidf}");
                            for (int i = 0; i < setidf.length; i++) {
                              for (int j = 0; j < organicvgt.length; j++) {
                                if (setidf[i] == "${organicvgt[j].id}") {
                                  savedataproductf.add(organicvgt[j].title);
                                  savedataproductf.add(organicvgt[j].price);
                                  savedataproductf.add(organicvgt[j].image);
                                  savedataproductf.add(organicvgt[j].phanloai);
                                }
                              }
                            }
