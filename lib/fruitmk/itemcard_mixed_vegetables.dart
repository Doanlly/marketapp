...
class ItemcardMixedVegetables extends StatefulWidget {
  var press;

  ItemcardMixedVegetables(
      {Key? key, required this.mixedvegt, required this.press})
      : super(key: key);
  final Mixedvegetables mixedvegt;

  //get press => null;
  //Function press;

  @override
  State<ItemcardMixedVegetables> createState() =>
      _ItemcardMixedVegetablesState();
}

class _ItemcardMixedVegetablesState extends State<ItemcardMixedVegetables> {
                    ...
                    child: InkWell(
                        onTap: () {
                          ...
                          if (checkfavorrite == false) {
                            setidf.add("${widget.mixedvegt.id}");
                            //print("${Listitemproduc}");
                            print("setid: ${setidf}");
                            for (int i = 0; i < setidf.length; i++) {
                              for (int j = 0; j < mixedvegt.length; j++) {
                                if (setidf[i] == "${mixedvegt[j].id}") {
                                  savedataproductf.add(mixedvegt[j].title);
                                  savedataproductf.add(mixedvegt[j].price);
                                  savedataproductf.add(mixedvegt[j].image);
                                  savedataproductf.add(mixedvegt[j].phanloai);
                                }
                              }
                            }
