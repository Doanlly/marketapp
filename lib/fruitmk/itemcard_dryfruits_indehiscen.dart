...
class ItemcardIndehiscentDryF extends StatefulWidget {
  var press;
  ItemcardIndehiscentDryF(
      {Key? key, required this.indehis, required this.press})
      : super(key: key);
  final IndehiscenDryF indehis;
  //get press => null;
  //Function press;
  @override
  State<ItemcardIndehiscentDryF> createState() =>
      _ItemcardIndehiscentDryFState();
}

class _ItemcardIndehiscentDryFState extends State<ItemcardIndehiscentDryF> {
                    ...
                    child: InkWell(
                        onTap: () {
                          ...
                          if (checkfavorrite == false) {
                            setidf.add("${widget.indehis.id}");
                            //print("${Listitemproduc}");
                            print("setid: ${setidf}");
                            for (int i = 0; i < setidf.length; i++) {
                              for (int j = 0; j < indehisdf.length; j++) {
                                if (setidf[i] == "${indehisdf[j].id}") {
                                  savedataproductf.add(indehisdf[j].title);
                                  savedataproductf.add(indehisdf[j].price);
                                  savedataproductf.add(indehisdf[j].image);
                                  savedataproductf.add(indehisdf[j].phanloai);
                                }
                              }
                            }
